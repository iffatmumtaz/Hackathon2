...
//                   <Image src={item.imageUrl} alt={item.title} className="h-12 w-12 object-cover rounded-lg" />
//                   <div className="ml-4">
...
const Checkout = () => {
  ...
  const applyDiscount = () => {
    ...
    const total = getCartTotal(); // Get cart total once

    if (discount) {
      setDiscountError(""); // Clear any previous error
      if (discount.discountType === "percentage") {
        // Apply percentage discount
        setAppliedDiscount((discount.value / 100) * total);
      } else if (discount.discountType === "fixed") {
        // Apply fixed discount
        setAppliedDiscount(discount.value);
      }
